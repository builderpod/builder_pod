class QuestionSetsController < ApplicationController
  ...
  def index
    if params[:task_profile_id].nil?
      @question_sets = QuestionSet.all
    else
      @task_profile = TaskProfile.find(params[:task_profile_id])
      @question_sets = @task_profile.question_set
    end
  ...
  private
    ...
    def question_set_params
      params.require(:question_set).permit(:oid, :task_id, :task_oid, :original_task_oid, :set_id, :interview_type)
    end
end
